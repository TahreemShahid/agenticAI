...
class AgenticService:
    ...
    def _make_request(self, prompt: str) -> str:
        ...
        try:
            ...
            response = requests.post(
                self.ai_agent_url, headers=self.headers, json=payload, timeout=30
            )
    ...
    def _check_greeting(self, query: str) -> Optional[Dict]:
        ...
        for pattern_type, patterns in greeting_patterns.items():
            ...
            for pattern in patterns:
                if pattern in query_lower:
                    ...
                    print(
                        f"[GREETING DEBUG] ✅ MATCH FOUND! Pattern: '{pattern}' Type: {pattern_type}"
                    )
                    break
            if greeting_type:
                break
        ...
        if not greeting_type:
            print(
                f"[GREETING DEBUG] ❌ No greeting pattern matched for: '{query_lower}'"
            )
            return None
        ...
        self.memory.add_message(
            "assistant",
            response,
            {"task_type": "greeting", "matched_pattern": matched_pattern},
        )

        print(
            f"[GREETING DEBUG] ✅ Generated response for {greeting_type}: {response[:50]}..."
        )
...
How can I help you with one of these tasks?"""
            ...
            self.memory.add_message(
                "assistant", response, {"task_type": "out_of_scope"}
            )
    ...
    def classify_with_enhanced_routing(
        self, user_query: str, context_info: str = None
    ) -> Dict:
        ...
        if greeting_result:
            print(
                f"[Classification] ✅ GREETING DETECTED: {greeting_result['parameters']['greeting_type']}"
            )
...
""".strip()
    ...
    def extract_two_texts_from_query(
        self, query: str
    ...
    def process_query(
        ...
        classification = self.classify_with_enhanced_routing(
            user_query, str(context) if context else None
        )
