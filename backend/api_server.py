...
import uvicorn, json, shutil, os, atexit, signal, sys, pickle, hashlib
...
async def handle_summarization_task(query: str, classification: Dict) -> Tuple[str, List[str]]:
    ...
    try:
        ...
        if agentic_service.memory.pdf_context:
            ...
            if recent_pdf in vector_stores:
                text_content = "\n".join([d.page_content for d in vector_stores[recent_pdf].docstore._dict.values()])
                cfg = load_config()
                summary = summarize(text_content, cfg["AI_Agent_URL"], cfg["API_KEY"], "brief")
        ...
        if text_to_summarize:
            cfg = load_config()
            summary = summarize(text_to_summarize, cfg["AI_Agent_URL"], cfg["API_KEY"], "brief")
            response = f"📝 **Text Summary:**\n\n{summary}"
            return response, []
        
        return "❌ I need text content or a PDF to summarize. Please provide text or upload a PDF first.", []
        
    except Exception as e:
        return f"❌ Summarization error: {str(e)}", []

async def handle_comparison_task(query: str, classification: Dict) -> Tuple[str, List[str]]:
    ...
    try:
        ...
        if len(agentic_service.memory.pdf_context) >= 2:
            pdf1, pdf2 = agentic_service.memory.pdf_context[-2], agentic_service.memory.pdf_context[-1]
            if pdf1 in vector_stores and pdf2 in vector_stores:
                text1 = "\n".join([d.page_content for d in vector_stores[pdf1].docstore._dict.values()])
                text2 = "\n".join([d.page_content for d in vector_stores[pdf2].docstore._dict.values()])
                cfg = load_config()
                comparison = compare_texts(text1, text2, "comprehensive", cfg["AI_Agent_URL"], cfg["API_KEY"])
        ...
        return "❌ I need two texts or two PDFs to compare. Please provide them separated by a clear line break or upload two PDFs.", []
...
async def handle_rag_task(query: str, classification: Dict) -> Tuple[str, List[str]]:
    ...
    try:
        ...
        cfg = load_config()
        llm = LLM(secret_key=cfg["API_KEY"], non_stream_url=cfg["AI_Agent_URL"],
...
async def lifespan(app: FastAPI):
    ...
    print(f"[Startup] Initialized with {len(uploaded_files)} files and {len(vector_stores)} vector stores")
...
# ===== FastAPI app setup =====
app = FastAPI(
    title="Complete Agentic PDF AI API - FIXED",
    version="3.1",
    lifespan=lifespan
)
...
async def unified_query_handler(request: SimpleQueryRequest):
    ...
    try:
        ...
        classification_result = agentic_service.classify_with_enhanced_routing(query, str(context_info))
        ...
        if task_type == "greeting":
            ...
            response_text, sources = await handle_summarization_task(query, classification_result)
            agentic_service.add_response_to_memory(response_text, task_type)
            
        elif task_type == "comparison":
            response_text, sources = await handle_comparison_task(query, classification_result)
            agentic_service.add_response_to_memory(response_text, task_type)
...
async def agentic_query(req: AgenticQueryRequest):
    ...
    try:
        ...
        if any(word in query_lower for word in ["goodbye", "bye"]):
            response_text = "Goodbye for now! 👋 I'm still here if you need help with documents."
        ...
        if task_type == "greeting":
            response_text = classification_result.get("response", "Hello! How can I help you?")
            
        elif task_type == "out_of_scope":
            response_text = classification_result.get("response", """🚫 I can only help with:
📝 Summarization
📊 Comparison  
❓ PDF Q&A""")
        ...
        print(f"[AGENTIC-QUERY] ✅ Returning {task_type} response: {response_text[:100]}...")
...
async def upload_pdfs(files: List[UploadFile] = File(...)):
    ...
    for i, file in enumerate(files):
        print(f"[UPLOAD] Processing file {i+1}: {getattr(file, 'filename', 'no-filename')}")
        ...
        try:
            ...
            if file_size_mb > 50:
                errors.append(f"{file.filename}: File too large ({file_size_mb:.1f}MB). Maximum size is 50MB.")
                continue
            ...
            if agentic_service:
                ...
                agentic_service.memory.add_message("system", f"PDF uploaded: {safe_filename}")
            
            print(f"[UPLOAD] ✅ Successfully processed {safe_filename} ({file_size_mb:.1f}MB)")
...
async def upload_and_query(
    ...
    try:
        ...
        query_request = AgenticQueryRequest(query=query, context={"upload_context": upload_result})
        ...
        raise HTTPException(status_code=500, detail=f"Upload and query failed: {str(e)}")
...
async def debug_greeting_test():
    ...
    for query in test_queries:
        ...
        results[query] = {
            ...
            "type": result.get("parameters", {}).get("greeting_type") if result else None,
            "response": result.get("response", "")[:100] + "..." if result else "No response"
        }
